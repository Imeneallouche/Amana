...
class User(AbstractUser):
    ...
    user_permissions = models.ManyToManyField(
        "auth.Permission", related_name="custom_user_permissions_set", null=True
    )
...
class HelpRequest(models.Model):
    ...
    person_in_need = models.ForeignKey(
        PersonInNeed, on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    category = models.CharField(
        max_length=50,
        choices=[("Health", "Health"), ("Food", "Food"), ("Education", "Education")],
    )
    description = models.TextField()
